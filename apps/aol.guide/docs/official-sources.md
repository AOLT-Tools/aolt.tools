...
Live listings: AOL Guide loads JSON from `https://www.artofliving.org/india-search-course-api` with the same filters plus `limit` and `offset` (offset is a 1-based page number). One page is fetched per search. Listings are not stored. `dist` in the payload is meters.
